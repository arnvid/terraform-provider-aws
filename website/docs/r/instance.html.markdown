...
* `instance_type` - (Optional) The type of instance to start. Can be set here or in Launch Template. If type is not set here or in provided `launch_template` instance type will default to `m1.small`. Updates to this field will trigger a stop/start of the EC2 instance.
...
* `launch_template` - (Optional) Launch template specification to use to launch instances. See [Launch Template Specification](#launch-template-specification) below for more details.
